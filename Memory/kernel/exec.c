...
exec(char *path, char **argv)
{
  ...
  // Static allocation only for /init and sh
  p->ondemand = (strncmp(path, "/init", 5) == 0 || strncmp(path, "sh", 2) == 0) ? false : true;
  if (p->ondemand) {
  ...
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    ...
    if (p->ondemand) {
      ...
      // Update virtual address space to ensure correct memory layout
      // For statically loaded processes, sz updated using uvmalloc
